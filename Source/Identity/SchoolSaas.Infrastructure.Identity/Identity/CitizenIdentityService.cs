...
{
    public class CitizenIdentityService : IStaffIdentityService
    {
        private readonly FrontOfficeIdentityContext _dbContext;
        ...
        public CitizenIdentityService(FrontOfficeIdentityContext dbContext,
        ...
        public async Task<string> ForgotPassword(ForgotPasswordRequest request)
        {
            ...
            if (user == null)
            {
                return "The email is not associated with any user.";
            }

            var token = await _userManager.GeneratePasswordResetTokenAsync(user);
            var resetLink = GenerateResetLink( token, request.Email);
            var emailDto = new PasswordResetEmailDTO { ResetLink = resetLink };
            var templateModel = new TemplateViewModel<PasswordResetEmailDTO>(emailDto);

            await _emailSender.SendEmailAsync(
             request.Email,
             "Réinitialisation du mot de passe",
             TemplatesNames.Emails.PasswordForgotten,
             emailDto
         );
        ...
        {
            var baseUrl = "https://econsulatma-fo-dev.azurewebsites.net/"; // Hardcoded base URL
        ...
        public async Task<string> ResetPasswordRequest(ResetPasswordRequest request)
        {
            var user = await _userManager.FindByEmailAsync(request.Email);
            if (user == null)
            {
                return new string("The email is not associated with any user.");
            }

            var result = await _userManager.ResetPasswordAsync(user, request.ResetCode, request.NewPassword);
            if (result.Succeeded)
            {
                return new string("Password has been reset successfully.");
            }
            else
            {
                return new string("Invalid token or password reset failed.");
            }
        }
        ...
        public async Task<Result> ChangePasswordAsync(string userId, string oldPassword, string newPassword)
        {
            ...
            if (result.Succeeded)
            {
                ...
                try
                {
                    await _emailSender.SendEmailAsync<UserResetDto>(user.Email, "Password Reset", TemplatesNames.Emails.ResetPassword, Mailuser);
                }
                catch (Exception)
                {

                }
                // Convert the IdentityResult to a custom Result type and return it
